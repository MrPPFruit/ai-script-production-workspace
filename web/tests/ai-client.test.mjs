...
test("向同源 API 发送契约输入并透传 DeepSeek 结果", async () => {
  ...
  const result = await requestBreakdown(input, async (url, init) => {
    call = { url, init };
    return Response.json({ mode: "deepseek", requestId: "req-1", suggestions: [{ label: "码头" }] });
  });
...
test("透传明确 fallback，并拒绝不合格响应", async () => {
  const fallback = await requestBreakdown(input, async () => Response.json({ mode: "fallback", reason: "MISSING_API_KEY", requestId: "req-2", suggestions: [] }));
