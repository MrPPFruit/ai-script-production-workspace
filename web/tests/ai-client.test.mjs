...
test("向同源 API 发送契约输入并透传 DeepSeek 结果", async () => {
  ...
  const result = await requestBreakdown(input, async (url, init) => {
    call = { url, init };
    return Response.json({ mode: "deepseek", requestId: "req-1", suggestions: [{ taxonomy: "location", label: "码头", evidenceKind: "explicit", confidence: "high", evidence: [{ quote: "码头", kind: "explicit" }] }] });
  });
...
test("透传明确 fallback，并拒绝不合格响应", async () => {
  const fallback = await requestBreakdown(input, async () => Response.json({ mode: "fallback", reason: "MISSING_API_KEY", requestId: "req-2", suggestions: [{ taxonomy: "other", taxonomyNote: "演示", label: "人工审阅", evidenceKind: "explicit", confidence: "low", evidence: [{ quote: "暴风雨", kind: "explicit" }] }] }));
