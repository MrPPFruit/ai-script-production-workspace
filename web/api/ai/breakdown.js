...
const json = (res, status, value) => res.status(status).setHeader("content-type", "application/json; charset=utf-8").send(JSON.stringify(value));
...
function fallback(body, id, reason) {
  const quote = body.scene.text.trim().slice(0, Math.min(120, body.scene.text.trim().length));
  return { mode: "fallback", reason, requestId: id, suggestions: [{ taxonomy: "other", taxonomyNote: "演示建议", label: "人工审阅场次要素", description: "请依据原文确认是否需要建立生产实体。", evidenceKind: "explicit", confidence: "low", evidence: [{ quote, kind: "explicit" }] }] };
}
...
export default async function handler(req, res) {
  const id = requestId();
  if (req.method !== "POST") return json(res, 405, { error: "METHOD_NOT_ALLOWED", requestId: id });
  const raw = typeof req.body === "string" ? req.body : JSON.stringify(req.body ?? {});
  let body;
  try { body = typeof req.body === "string" ? JSON.parse(req.body) : req.body; } catch { return json(res, 400, { error: "INVALID_REQUEST", requestId: id }); }
  if (raw.length > LIMITS.body || !validRequest(body)) return json(res, 400, { error: "INVALID_REQUEST", requestId: id });
  if (!process.env.DEEPSEEK_API_KEY) return json(res, 200, fallback(body, id, "MISSING_API_KEY"));
  ...
  try {
    ...
    if (!upstream.ok) return json(res, 200, fallback(body, id, "UPSTREAM_ERROR"));
    const payload = JSON.parse((await upstream.json())?.choices?.[0]?.message?.content || "");
    const suggestions = normalize(payload, body);
    return suggestions ? json(res, 200, { mode: "deepseek", requestId: id, suggestions }) : json(res, 200, fallback(body, id, "INVALID_MODEL_OUTPUT"));
  } catch (error) {
    return json(res, 200, fallback(body, id, error?.name === "AbortError" ? "UPSTREAM_TIMEOUT" : "UPSTREAM_ERROR"));
  } finally {
    clearTimeout(timer);
  }
}
